...
class RCA_Calculator:
    ...
    def all_imp_specific(self, prods: list, df: pd.DataFrame, val: str, imp_country, exp_country) -> float:
        """        
        Return the sum of products importation from a exporter(<exp_country> single exporter or the world) to a importer(<imp_country>: single importer). The sum can be value or quantity (val).
        """
        all_prod = df[(df['k'].isin(prods)) & (df['i'] == exp_country) & (df['j'] == imp_country)].copy()
    ...
    def generate_xwj_specific(self, prods: list, folder_path, file, vals, country_code) -> list:
        """
        Generate xwj.csv file for final calculation. 
            'xwj' means total export value of all commodities from a country to country j.
        ...
        if country_code == "all":
            ...
            for exporter in exporter_code:
                for importer in importer_code:
                    row = [year, exporter, importer]
                    for val in vals:
                        country_single_imp = self.all_imp_specific(prods, selected_df, val, importer, exporter)
                        row.append(country_single_imp)
                    country_all_rows.append(row)
